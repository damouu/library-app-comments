...
it('maps arguments correctly and saves a comment', async () => {
    const dto = new CreateCommentDTO('Hello repo', uuid4(), uuid4(), 'Repo User', 'repo@example.com', 'https://avatar.test/img.png');
    ...
    expect(saved.content).toBe(dto.content);
    expect(saved.chapterUuid).toBe(dto.chapterUuid);
    expect(saved.memberCardUuid).toBe(dto.memberCardUuid);
    expect(saved.userName).toBe(dto.userName);
    expect(saved.userEmail).toBe(dto.userEmail);
    expect(saved.avatar_URL).toBe(dto.avatar_URL);
